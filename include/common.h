...
class cuboid {
public:
    explicit cuboid(ent& entity) { init(entity); };
    void update(ent& entity) { init(entity); };
    //explicit here means we're not converting bot_ent to a cuboid really. just semantics things from my understanding
    ...
    void draw(ImDrawList* dl, const ImU32 colour_prmry, const ImU32 colour_scnd, const float thickness) const;

    void world_to_screen(const glm::mat4 &view, const glm::mat4 &proj, const ImVec2 &window_size,
                         const ImVec2 &default_position);
    void init(ent& entity);
};

static_assert(Drawable<cuboid>, "cuboid must satisfy Drawable");
...
struct esp_data {
    std::vector<cuboid> _entity_cuboids{};
    std::mutex _cuboid_mex{};
};
